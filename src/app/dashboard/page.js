...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* AUTOMATION 3: Overdue alert banner — shown only when stale services exist */}
        ...
        <div className="bg-red-50 border border-red-200 rounded-2xl px-5 py-4 flex items-start gap-4">
          ...
          <div className="flex-1">
            <p className="font-bold text-red-700 text-sm">
              {stats.overdueCount} service
              {stats.overdueCount > 1 ? "s are" : " is"} overdue (pending &gt;24
              hours)
            </p>
            <div className="mt-2 flex flex-wrap gap-2">
              {overdueServices.slice(0, 4).map((s) => (
                <span
                  key={s.id}
                  className="text-xs px-3 py-1 bg-red-100 text-red-700 rounded-full border border-red-200"
                >
              ...
              {overdueServices.length > 4 && (
                <span className="text-xs px-3 py-1 bg-red-100 text-red-700 rounded-full border border-red-200">
                  +{overdueServices.length - 4} more
                </span>
              )}
            </div>
          </div>
      ...
      <div className="bg-gradient-to-r from-green-600 to-green-500 rounded-2xl p-6 text-white flex items-center justify-between">
        <div>
          ...
          <p className="text-green-100 mt-1">
            {config?.ownerName} · {config?.type?.toUpperCase()}
          </p>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            >
              <div
                className={`w-11 h-11 rounded-xl ${s.bg} flex items-center justify-center text-xl mb-4`}
              >
                {s.icon}
              </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
            {[
              ...
              >
                ...
                <span className={`text-sm font-medium ${a.color}`}>
                  {a.label}
                </span>
          ...
          {pendingServices.length === 0 ? (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {["Customer", "Service", "Qty", "Price", "Status"].map(
                    (h) => (
                      <th
                        key={h}
